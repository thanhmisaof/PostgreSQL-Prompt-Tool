...
- **Phím Tắt Viết Code (Snippet)**: Gõ các phím tắt siêu tốc để sinh ra template code phức tạp 
+ ssf -> SELECT * FROM
+ ii -> INSERT INTO  () VALUES ()
+ uj -> UPDATE SET = WHERE 
+ del -> DELETE FROM WHERE
+ jj -> JOIN  ON
+ lj -> LEFT OUTER JOIN  ON 
+ rj -> RIGHT OUTER JOIN  ON 
+ fj -> FULL OUTER JOIN  ON 
+ iff -> IF  THEN END IF
+ ife -> IF  THEN ELSE END IF
+ for -> FOR _rec IN LOOP END LOOP;
+ whl -> WHILE  LOOP END LOOP;
+ cur -> 
	DECLARE
    		_cur CURSOR FOR
	...
	END;
+ crf -> CREATE FUNCTION
+ crp -> CREATE OR REPLACE PROCEDURE
+ crv -> CREATE OR REPLACE VIEW
+ cte -> WITH cte_name AS
+ dbe -> DO $$
	DECLARE _parameter <type>;
	BEGIN
		SELECT col1 INTO _parameter FROM tablename;
		RAISE NOTICE '%', _parameter;
	END $$;
+ rai -> RAISE NOTICE '_Key = %', _Key;
+ tr1 -> PERFORM dbo.ufn_TraceQuery($$SELECT * FROM tablename$$);
+ tr2 -> PERFORM dbo.ufn_TraceQuery(
		FORMAT($$
			SELECT *
			FROM tablename
			WHERE colname = %s;
			$$, parameter1.. (biến truyền vào ở đây)))
+ fet -> FETCH ALL FROM <ref1>;
...
7. Mở phần mềm Database của bạn (**DBeaver**, **pgAdmin**, **Datagrip**), gõ `af ten_ham_cua_ban` và Tab nhấn **`Ctrl+SPACE`**.
