...
- **Truy Cập Gốc (Object Binding)**: Chỉ cần gõ `af ten_ham` và nhấn `Ctrl+Enter`, nội dung code của hàm (function), thủ tục (procedure) hoặc bảng (table) từ Database sẽ lập tức được chèn vào cửa sổ code của bạn.
...
7. Mở phần mềm Database của bạn (**DBeaver**, DBeaver CE), gõ `af ten_ham_cua_ban` và nhấn **`Ctrl+SPACE`**.
